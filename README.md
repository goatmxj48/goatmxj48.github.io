...
|---|---|---|
| Projects 카드 7 · AI 개발 환경 | `p11-ai.png` | 테스트케이스 생성 명령어로 만든 엑셀 |
| Previous Projects 카드 3 · 에듀윌 | `p06-eduwill.png` | SmartTM 관리자 화면 |
| 상세 · 에듀윌 | `p06-eduwill-01.png` `p06-eduwill-02.png` | 관리자 화면 / IP 추적 |
...
페이지 차례는 이력서와 같습니다 — About Me · Skill Set · Work Experience · Projects · Previous Projects · Key Results · Education & Certification · Contact.

| 고치고 싶은 것 | 찾을 곳 |
|---|---|
| **테마 색** | 파일 맨 위 `:root` 의 `--accent` — 이 한 줄로 사이트 전체 색이 바뀝니다 |
| 카드 문구 | 해당 한글을 그대로 검색 |
| 상세(모달) 내용 | 파일 아래쪽 `<div hidden>` 안의 `id="d1"` ~ `id="d11"` |
| 섹션 이름 · 경계선 | `<div class="sec-line">` — 영문 이름 · 우리말 이름 · 오른쪽 보조 문구가 한 줄에 들어가고 그 아래 경계선이 그어집니다 |
...
카드를 눌러 상세를 열고 그 안의 이미지를 누르면 크게 보기가 열립니다.
같은 상세 안의 이미지는 하나로 묶여, 크게 보기에서 바로 앞뒤로 넘길 수 있습니다.

| 하고 싶은 것 | 하는 방법 |
|---|---|
| 다음 · 이전 이미지 | 좌우 화살표 버튼 · 키보드 `←` `→` · 옆으로 밀기(스와이프) |
| 지금 몇 번째인지 | 아래 조작줄의 `2 / 4` 표시 |
| 확대 · 축소 | 휠 · 핀치 · `+` `−` 버튼 · 키보드 `+` `-` |
| 화면 맞춤 | `화면 맞춤` 버튼 · 키보드 `0` |
| 닫기 | `✕` · 이미지 바깥 클릭 · `Esc` |

- 이미지가 한 장뿐인 상세에서는 화살표와 번호 표시가 나오지 않습니다
- 마지막 장에서 다음을 누르면 첫 장으로 돌아옵니다
- 확대한 상태에서 미는 것은 화면 이동이라 이미지가 넘어가지 않습니다. 화살표와 `←` `→` 는 확대 중에도 넘어갑니다
- 넘기면 배율은 다시 화면 맞춤으로 돌아갑니다

밀기(스와이프)로 넘어가는 거리는 `<script>` 안의 이 값입니다.

```js
var SWIPE = 60;
```

`60` 은 60 px 이상 옆으로 밀면 넘어간다는 뜻입니다. 잘못 넘어가는 느낌이면 `90` 정도로 올리고,
살짝만 밀어도 넘어가게 하려면 `40` 정도로 내립니다.
